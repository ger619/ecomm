class AdminController < ApplicationController
  ...
  def index
    ...
    @quick_stats = {
      ...
      average_sales: Order.where(created_at: Time.now.midnight..Time.now).average(:total).round,
    ...
    @revenue_by_day = @order_by_day.map { |date, orders| [date.strftime('%A '), orders.sum(&:total)] }
    ...
    ordered_days_with_current_last = days_of_the_week[current_day_index..] + days_of_the_week[0...current_day_index]
    complete_order_array_with_current_last = ordered_days_with_current_last.map { |day| [day, data_hash.fetch(day, 0)] }
